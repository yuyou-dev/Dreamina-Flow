...
- Dreamina CLI 参数面对齐当前 `dreamina -h`：图片批量 `generate_num`、Seedance 1.x/2.0 mini，以及 `seedance2.0_vip` 的 `4k` 视频都已进入 adapter 校验。

## 项目价值速览

![Workflow Studio OSS 能力看板插画，展示节点化 Dreamina CLI、浏览器运行 workflow、导入导出 workflow 文件、示例流程和登录恢复能力](images/highlights.png)
...
![Workflow Studio OSS 四层结构插画，展示 studio-web、studio-api、dreamina-adapter 和 workflow-core 的职责分工与调用方向](images/architecture.png)
...
![Workflow Studio OSS 运行链路插画，展示画布编辑、节点运行、OAuth Device Flow 登录恢复与任务结果回写](images/workflow.png)
...
- 登录 UI 以 OAuth Device Flow 为主线，不再依赖旧的浏览器 callback 或手动导入登录形式。

## 快速开始

![Workflow Studio OSS 快速开始插画，展示安装 Dreamina CLI、npm install、npm run dev、默认端口和环境要求](images/quick-start.png)

这个仓库当前只在 `macOS` 上验证，默认端口固定为 `3000`（Web）和 `3100`（API）。在本地环境满足 `Node.js 20+`、`npm 10+` 和可用 `python3` 之后，可以直接按下面的顺序启动。

Dreamina CLI 的新版登录已经切换为 OAuth Device Flow，因此首次运行时更应该先确认 CLI 能正常执行，再启动 Studio。
...
- `node ./scripts/migrate-workflow-schema.mjs --check` 用于确认 workflow schema 没有偏移。
...
这份 README 草稿基于以下仓库文件整理：

- README.md
- docs/technical-overview.md
- docs/workflow-json-format.md
- docs/comic-project-intro/style-pack.md
- package.json
- apps/studio-api/package.json
- apps/studio-web/package.json
- packages/workflow-core/package.json
- packages/dreamina-adapter/package.json
